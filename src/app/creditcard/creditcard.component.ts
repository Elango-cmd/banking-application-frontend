...
export class CreditcardComponent {
  ...
  constructor(private http: HttpClient,private fb: FormBuilder,private router: Router, private customerService: CustomerService) {
    debugger;
    const paymentIdAmount= this.customerService.getPaymentAmount();
    console.log(paymentIdAmount);
  ...
  onCredit() {
    ...
    if (this.creditCardForm.valid) {
      ...
      console.log(paymentIdKey);
      const url = `http://localhost:9090/api/v1/customerBankingPayment/paymentDetail?paymentId=${paymentIdKey}`;
      ...
      .subscribe((res: any) => {
        if (res) {
          ...
          alert("welcome again "+this.customerService.getCustomerName()+" Transaction completed successfully!");
